#!/usr/bin/env python3
"""Durable GPU0--3 launcher for the frozen V8 official boundary-head study."""
...
from mal2026.iterative_official_boundary_models import candidate_specs  # noqa: E402
from mal2026.iterative_official_boundary_protocol import (  # noqa: E402
    CONFIG_PATH, RUN_ID, load_protocol, validate_bound_inputs,
)
from mal2026.iterative_official_boundary_runner import (  # noqa: E402
...
TMUX_SESSION = "mal2026-v8-official-boundary"
...
def _visible_gpu() -> int:
    ...
    if visible not in {str(gpu) for gpu in AUTHORIZED_GPUS}:
        raise RuntimeError("V8 worker requires one authorized physical GPU 0..3")
    return int(visible)
...
def _write_task_card(snapshot: list[dict[str, object]]) -> None:
    ...
    _atomic_json(PUBLIC_ROOT / "task_card.json", {
        "schema_version": "mal2026-iterative-official-boundary-task-card-v8",
        "run_id": RUN_ID,
        "created_at": datetime.now(timezone.utc).isoformat(),
        "git_sha_at_launch": git_sha,
        "config_path": str(CONFIG_PATH),
        "config_sha256": _sha256(ROOT / CONFIG_PATH),
        "model_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_boundary_models.py"),
        "protocol_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_boundary_protocol.py"),
        "selection_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_boundary_selection.py"),
        "runner_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_boundary_runner.py"),
        ...
        "v7_aggregate_sha256": audit.v7_aggregate_sha256,
        "v7_result_role": protocol.raw["lineage"]["v7_result_role"],
        ...
        "fresh_residual_and_boundary_heads_each_candidate_and_fold": True,
...
def _smoke_complete() -> bool:
    ...
    try:
        ...
        return False
    return result.get("schema_version") == "mal2026-iterative-official-boundary-smoke-v8" and result.get("status") == "completed" and result.get("gpu") == 0 and len(result.get("candidates", ())) == 3
...
def _smoke_controller() -> None:
    ...
    if code:
        ...
        raise RuntimeError(f"V8 GPU0 smoke failed: exit {code}")
...
def _outer_controller() -> None:
    ...
    while pending or running:
        ...
        if running and not finished:
            time.sleep(2)
    if failures:
        raise RuntimeError(f"V8 outer stage failed: {failures}")
...
def _full() -> None:
    ...
    if conflicts:
        raise RuntimeError(f"pre-existing compute process detected; V8 launch refused: {conflicts}")
...
def main() -> None:
    ...
    if args.launch:
        ...
        if _visible_gpu() != 0:
            raise RuntimeError("V8 smoke worker must use physical GPU0")
