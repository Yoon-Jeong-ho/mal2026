#!/usr/bin/env python3
"""Durable GPU0--3 launcher for the frozen V11 official balanced-boundary-head study."""
...
from mal2026.iterative_official_balanced_boundary_models import candidate_specs  # noqa: E402
from mal2026.iterative_official_balanced_boundary_protocol import (  # noqa: E402
    CONFIG_PATH, RUN_ID, load_protocol, validate_bound_inputs,
)
from mal2026.iterative_official_balanced_boundary_runner import (  # noqa: E402
...
TMUX_SESSION = "mal2026-v11-official-balanced-boundary"
...
def _ledger(event: str, **fields: object) -> None:
    ...
    with path.open("a", encoding="utf-8") as stream:
        stream.write(json.dumps({
            "schema_version": "mal2026-iterative-official-balanced-boundary-ledger-event-v11",
            "run_id": RUN_ID,
            "at_epoch": time.time(),
            "event": event,
            **fields,
...
def _visible_gpu() -> int:
    ...
    if visible not in {str(gpu) for gpu in AUTHORIZED_GPUS}:
        raise RuntimeError("V11 worker requires one authorized physical GPU 0..3")
    return int(visible)
...
def _write_task_card(snapshot: list[dict[str, object]]) -> None:
    ...
    _atomic_json(PUBLIC_ROOT / "task_card.json", {
        "schema_version": "mal2026-iterative-official-balanced-boundary-task-card-v11",
        "run_id": RUN_ID,
        "created_at": datetime.now(timezone.utc).isoformat(),
        "git_sha_at_launch": git_sha,
        "config_path": str(CONFIG_PATH),
        "config_sha256": _sha256(ROOT / CONFIG_PATH),
        "model_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_balanced_boundary_models.py"),
        "protocol_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_balanced_boundary_protocol.py"),
        "selection_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_balanced_boundary_selection.py"),
        "runner_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_balanced_boundary_runner.py"),
        ...
        "luna_generation_plan_sha256": protocol.raw["lineage"]["luna_generation_plan_sha256"],
        "luna_request_sha256": protocol.raw["lineage"]["luna_request_sha256"],
        ...
        "v10_aggregate_sha256": protocol.raw["lineage"]["v10_aggregate_sha256"],
        "v10_completion_sha256": protocol.raw["lineage"]["v10_completion_sha256"],
        "v10_result_role": protocol.raw["lineage"]["v10_result_role"],
...
def _smoke_complete() -> bool:
    ...
    try:
        ...
        return False
    return result.get("schema_version") == "mal2026-iterative-official-balanced-boundary-smoke-v11" and result.get("status") == "completed" and result.get("gpu") == 0 and len(result.get("candidates", ())) == 3
...
def _smoke_controller() -> None:
    ...
    if code:
        ...
        raise RuntimeError(f"V11 GPU0 smoke failed: exit {code}")
...
def _outer_controller() -> None:
    ...
    while pending or running:
        ...
        if running and not finished:
            time.sleep(2)
    if failures:
        raise RuntimeError(f"V11 outer stage failed: {failures}")
...
def _full() -> None:
    ...
    if conflicts:
        raise RuntimeError(f"pre-existing compute process detected; V11 launch refused: {conflicts}")
...
def _progress() -> dict[str, object]:
    ...
    return {
        "schema_version": "mal2026-iterative-official-balanced-boundary-progress-summary-v11",
        "run_id": RUN_ID,
...
def main() -> None:
    ...
    if args.launch:
        ...
        if _visible_gpu() != 0:
            raise RuntimeError("V11 smoke worker must use physical GPU0")
