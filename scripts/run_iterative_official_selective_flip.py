#!/usr/bin/env python3
"""Durable GPU0--3 launcher for the frozen V9 official selective-flip-head study."""
...
from mal2026.iterative_official_selective_flip_models import candidate_specs  # noqa: E402
from mal2026.iterative_official_selective_flip_protocol import (  # noqa: E402
    CONFIG_PATH, RUN_ID, load_protocol, validate_bound_inputs,
)
from mal2026.iterative_official_selective_flip_runner import (  # noqa: E402
...
TMUX_SESSION = "mal2026-v9-official-selective-flip"
...
def _visible_gpu() -> int:
    ...
    if visible not in {str(gpu) for gpu in AUTHORIZED_GPUS}:
        raise RuntimeError("V9 worker requires one authorized physical GPU 0..3")
    return int(visible)
...
def _write_task_card(snapshot: list[dict[str, object]]) -> None:
    ...
    _atomic_json(PUBLIC_ROOT / "task_card.json", {
        "schema_version": "mal2026-iterative-official-selective-flip-task-card-v9",
        "run_id": RUN_ID,
        "created_at": datetime.now(timezone.utc).isoformat(),
        "git_sha_at_launch": git_sha,
        "config_path": str(CONFIG_PATH),
        "config_sha256": _sha256(ROOT / CONFIG_PATH),
        "model_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_selective_flip_models.py"),
        "protocol_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_selective_flip_protocol.py"),
        "selection_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_selective_flip_selection.py"),
        "runner_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_selective_flip_runner.py"),
        ...
        "v8_aggregate_sha256": audit.v8_aggregate_sha256,
        "v8_result_role": protocol.raw["lineage"]["v8_result_role"],
        ...
        "fresh_residual_and_selective_heads_each_candidate_and_fold": True,
...
def _smoke_complete() -> bool:
    ...
    try:
        ...
        return False
    return result.get("schema_version") == "mal2026-iterative-official-selective-flip-smoke-v9" and result.get("status") == "completed" and result.get("gpu") == 0 and len(result.get("candidates", ())) == 3
...
def _smoke_controller() -> None:
    ...
    if code:
        ...
        raise RuntimeError(f"V9 GPU0 smoke failed: exit {code}")
...
def _outer_controller() -> None:
    ...
    while pending or running:
        ...
        if running and not finished:
            time.sleep(2)
    if failures:
        raise RuntimeError(f"V9 outer stage failed: {failures}")
...
def _full() -> None:
    ...
    if conflicts:
        raise RuntimeError(f"pre-existing compute process detected; V9 launch refused: {conflicts}")
...
def main() -> None:
    ...
    if args.launch:
        ...
        if _visible_gpu() != 0:
            raise RuntimeError("V9 smoke worker must use physical GPU0")
