#!/usr/bin/env python3
"""Durable GPU0--3 launcher for the final nested v5 learner study."""
...
from mal2026.iterative_tail_learner_protocol import (  # noqa: E402
    CONFIG_PATH,
    RUN_ID,
    load_protocol,
...
from mal2026.iterative_tail_learner_runner import (  # noqa: E402
    PUBLIC_ROOT,
    RESTRICTED_ROOT,
...
TMUX_SESSION = "mal2026-v5-learner"
    ...
    evidence = [
        {
            ...
            "timestamp": timestamp,
            "hypothesis": "fresh embedding-plus-score-blind learners may add signal absent from V4 score routing",
            "predicted_direction": "all_seven_inner_gates_and_the_fixed_final_gate_improve",
            "falsification_condition": "no candidate passes the seven-gate inner AND or the final nested gate",
            "accepted_or_rejected_rationale": "accepted_as_a_separate_user-authorized adaptive V5 preregistration",
            "config_sha256": config_sha,
        },
        {
            "agent_role": "modeling",
            "timestamp": timestamp,
            "hypothesis": "five bounded learner families can improve exact R0 from frozen embeddings and score-blind evidence",
            "predicted_direction": "joint_macro_equal_group_low_high_and_mid_boundary_improvement",
            "falsification_condition": "all twenty fixed candidates fail the inner gate or fresh outer refit loses the signal",
            "accepted_or_rejected_rationale": "accepted_as_exact twenty-candidate inventory before V5 outer results",
            "config_sha256": config_sha,
        },
        {
            ...
            "predicted_direction": "outer_predictions_once_after_candidate_spec_freeze",
            "falsification_condition": "any D or O target reaches candidate fitting, or concatenated outer results trigger selection",
            "accepted_or_rejected_rationale": "accepted with descriptive-only claims and permanent V4 inventory/weight freeze",
            "config_sha256": config_sha,
        },
    ]
    _atomic_json(PUBLIC_ROOT / "task_card.json", {
        "schema_version": "mal2026-iterative-tail-learner-task-card-v5",
        "run_id": RUN_ID,
        "git_sha_at_launch": git_sha,
        "config_path": str(CONFIG_PATH),
        "config_sha256": config_sha,
        "model_source_sha256": _sha256(ROOT / "src/mal2026/iterative_tail_learner_models.py"),
        "protocol_source_sha256": _sha256(ROOT / "src/mal2026/iterative_tail_learner_protocol.py"),
        "runner_source_sha256": _sha256(ROOT / "src/mal2026/iterative_tail_learner_runner.py"),
        "selection_source_sha256": _sha256(ROOT / "src/mal2026/iterative_tail_learner_selection.py"),
        ...
        "v1_historical_promotion_sha256": audit.historical_v1_promotion_sha256,
        ...
        "v4_historical_aggregate_sha256": audit.historical_v4_aggregate_sha256,
        ...
        "adaptive_after_v1_v2_v3_v4_observed": True,
        "v4_inventory_and_learned_weights_permanently_frozen": True,
        "v4_stop_supersession_authorization": protocol.raw["v4_stop_supersession"],
...
def _outer_complete(outer: int) -> bool:
    ...
    try:
        ...
        return False
    return (
        result.get("status") == "completed" and result.get("candidate_count") == 20
...
def _smoke_complete() -> bool:
    ...
    try:
        ...
        return False
    return (
        result.get("schema_version") == "mal2026-iterative-tail-learner-smoke-v5"
        and result.get("status") == "completed" and result.get("gpu") == 0
        and len(result.get("family_smokes", ())) == 5
    )
...
def _full() -> None:
    ...
    _ledger(
        ...
        v5_inventory_frozen=aggregate["v5_inventory_frozen"],
    )
    print(json.dumps({
        "run_id": RUN_ID,
        "status": "completed",
...
def _progress() -> dict[str, object]:
    ...
    for outer in range(5):
        ...
        if path.is_file():
            ...
            outers.append({
                ...
                "completed_candidates": 20, "percent": 100.0,
            })
        else:
            ...
            if progress_path.is_file():
                ...
                outers.append({
                    ...
                    "completed_candidates": count, "candidate_count": 20,
                ...
                outers.append({"outer_fold": outer, "status": "pending", "completed_candidates": 0,
                               "candidate_count": 20, "percent": 0.0})
    ...
    return {
        ...
        "nested_search_total_candidates": 100,
        "nested_search_percent": 100.0 * completed_candidates / 100.0,
        "outers": outers,
