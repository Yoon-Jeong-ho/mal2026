#!/usr/bin/env python3
"""Durable GPU0--3 launcher for the frozen V12 rationale-semantic study."""
...
from mal2026.iterative_official_rationale_semantic_models import candidate_specs  # noqa: E402
from mal2026.iterative_official_rationale_semantic_protocol import (  # noqa: E402
    CONFIG_PATH, RUN_ID, load_protocol, validate_bound_inputs,
)
from mal2026.iterative_official_rationale_semantic_runner import (  # noqa: E402
    PUBLIC_ROOT, RESTRICTED_ROOT, SEED, aggregate_outer_results, gpu0_smoke, run_outer_fold,
)


PYTHON = ROOT / ".venv-standard" / "bin" / "python"
AUTHORIZED_GPUS = (0, 1, 2, 3)
TMUX_SESSION = "mal2026-v12-official-rationale-semantic"
...
def _ledger(event: str, **fields: object) -> None:
    ...
    with path.open("a", encoding="utf-8") as stream:
        stream.write(json.dumps({
            "schema_version": "mal2026-iterative-official-rationale-semantic-ledger-event-v12",
            "run_id": RUN_ID,
            "at_epoch": time.time(),
            "event": event,
            **fields,
...
def _visible_gpu() -> int:
    ...
    if visible not in {str(gpu) for gpu in AUTHORIZED_GPUS}:
        raise RuntimeError("V12 worker requires one authorized physical GPU 0..3")
    return int(visible)
...
def _write_task_card(snapshot: list[dict[str, object]]) -> None:
    ...
    _atomic_json(PUBLIC_ROOT / "task_card.json", {
        "schema_version": "mal2026-iterative-official-rationale-semantic-task-card-v12",
        "run_id": RUN_ID,
        "created_at": datetime.now(timezone.utc).isoformat(),
        "git_sha_at_launch": git_sha,
        "config_path": str(CONFIG_PATH),
        "config_sha256": _sha256(ROOT / CONFIG_PATH),
        "model_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_rationale_semantic_models.py"),
        "protocol_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_rationale_semantic_protocol.py"),
        "selection_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_rationale_semantic_selection.py"),
        "runner_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_rationale_semantic_runner.py"),
        ...
        "terra_candidate_manifest_sha256": protocol.raw["lineage"]["terra_candidate_manifest_sha256"],
        "terra_candidate_rows_sha256": protocol.raw["lineage"]["terra_candidate_rows_sha256"],
        "luna_candidate_manifest_sha256": protocol.raw["lineage"]["luna_candidate_manifest_sha256"],
        "luna_candidate_rows_sha256": protocol.raw["lineage"]["luna_candidate_rows_sha256"],
        "official_system_prompt_sha256": protocol.raw["lineage"]["official_system_prompt_sha256"],
        "qwen_model_id": protocol.raw["lineage"]["qwen_model_id"],
        "qwen_model_revision": protocol.raw["lineage"]["qwen_model_revision"],
        "qwen_model_config_sha256": protocol.raw["lineage"]["qwen_model_config_sha256"],
        "generated_feature_manifest_sha256": protocol.raw["lineage"]["generated_feature_manifest_sha256"],
        "generated_feature_rows_sha256": protocol.raw["lineage"]["generated_feature_rows_sha256"],
        "fold_assignment_fingerprint": audit.fold_fingerprint,
        "v11_aggregate_sha256": protocol.raw["lineage"]["v11_aggregate_sha256"],
        "v11_completion_sha256": protocol.raw["lineage"]["v11_completion_sha256"],
        "v11_result_role": protocol.raw["lineage"]["v11_result_role"],
        ...
        "fresh_residual_and_class_balanced_head_each_candidate_and_fold": True,
...
def _outer_complete(outer: int) -> bool:
    ...
    return (
        ...
        and result.get("candidate_count") == 3
...
def _smoke_complete() -> bool:
    ...
    try:
        ...
        return False
    return result.get("schema_version") == "mal2026-iterative-official-rationale-semantic-smoke-v12" and result.get("status") == "completed" and result.get("gpu") == 0 and len(result.get("candidates", ())) == 3
...
def _smoke_controller() -> None:
    ...
    if code:
        ...
        raise RuntimeError(f"V12 GPU0 smoke failed: exit {code}")
...
def _outer_controller() -> None:
    ...
    while pending or running:
        ...
        if running and not finished:
            time.sleep(2)
    if failures:
        raise RuntimeError(f"V12 outer stage failed: {failures}")
...
def _full() -> None:
    ...
    if conflicts:
        raise RuntimeError(f"pre-existing compute process detected; V12 launch refused: {conflicts}")
...
def _progress() -> dict[str, object]:
    ...
    return {
        "schema_version": "mal2026-iterative-official-rationale-semantic-progress-summary-v12",
        "run_id": RUN_ID,
        "smoke_completed": _smoke_complete(),
        "nested_search_completed_candidates": completed,
        "nested_search_total_candidates": 15,
        "nested_search_percent": 100.0 * completed / 15.0,
        "outers": outers,
...
def main() -> None:
    ...
    if args.launch:
        ...
        if _visible_gpu() != 0:
            raise RuntimeError("V12 smoke worker must use physical GPU0")
