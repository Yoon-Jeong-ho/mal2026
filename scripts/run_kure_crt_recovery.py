...
from mal2026.kure_crt_recovery import KURECRTRecoveryConfig, aggregate, run


def main() -> None:
    parser = argparse.ArgumentParser(description="Run train-only KURE CORAL cRT recovery.")
    ...
    config = KURECRTRecoveryConfig.from_json(args.config, require_dependencies=not args.validate_only)
    if args.aggregate:
