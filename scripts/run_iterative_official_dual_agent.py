#!/usr/bin/env python3
"""Durable GPU0--3 launcher for the frozen V10 official dual-agent-head study."""
...
from mal2026.iterative_official_dual_agent_models import candidate_specs  # noqa: E402
from mal2026.iterative_official_dual_agent_protocol import (  # noqa: E402
    CONFIG_PATH, RUN_ID, load_protocol, validate_bound_inputs,
)
from mal2026.iterative_official_dual_agent_runner import (  # noqa: E402
...
TMUX_SESSION = "mal2026-v10-official-dual-agent"
...
def _visible_gpu() -> int:
    ...
    if visible not in {str(gpu) for gpu in AUTHORIZED_GPUS}:
        raise RuntimeError("V10 worker requires one authorized physical GPU 0..3")
    return int(visible)
...
def _write_task_card(snapshot: list[dict[str, object]]) -> None:
    ...
    _atomic_json(PUBLIC_ROOT / "task_card.json", {
        "schema_version": "mal2026-iterative-official-dual-agent-task-card-v10",
        "run_id": RUN_ID,
        "created_at": datetime.now(timezone.utc).isoformat(),
        "git_sha_at_launch": git_sha,
        "config_path": str(CONFIG_PATH),
        "config_sha256": _sha256(ROOT / CONFIG_PATH),
        "model_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_dual_agent_models.py"),
        "protocol_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_dual_agent_protocol.py"),
        "selection_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_dual_agent_selection.py"),
        "runner_source_sha256": _sha256(ROOT / "src/mal2026/iterative_official_dual_agent_runner.py"),
        ...
        "post_generation_binding_state": protocol.raw["post_generation_binding"]["state"],
        "v9_aggregate_sha256": protocol.raw["lineage"]["v9_aggregate_sha256"],
        "v9_completion_sha256": protocol.raw["lineage"]["v9_completion_sha256"],
        "v9_result_role": protocol.raw["lineage"]["v9_result_role"],
        ...
        "fresh_residual_and_optional_heads_each_candidate_and_fold": True,
...
def _smoke_complete() -> bool:
    ...
    try:
        ...
        return False
    return result.get("schema_version") == "mal2026-iterative-official-dual-agent-smoke-v10" and result.get("status") == "completed" and result.get("gpu") == 0 and len(result.get("candidates", ())) == 3
...
def _smoke_controller() -> None:
    ...
    if code:
        ...
        raise RuntimeError(f"V10 GPU0 smoke failed: exit {code}")
...
def _outer_controller() -> None:
    ...
    while pending or running:
        ...
        if running and not finished:
            time.sleep(2)
    if failures:
        raise RuntimeError(f"V10 outer stage failed: {failures}")
...
def _full() -> None:
    ...
    if conflicts:
        raise RuntimeError(f"pre-existing compute process detected; V10 launch refused: {conflicts}")
...
def main() -> None:
    ...
    if args.launch:
        ...
        if _visible_gpu() != 0:
            raise RuntimeError("V10 smoke worker must use physical GPU0")
