...
from mal2026.iterative_official_agent_stack_models import AgentStackFitResult, FEATURE_DIM, candidate_specs
from mal2026.iterative_official_agent_stack_runner import AgentStackData, _candidate_oof
from mal2026.iterative_tail_runner import ExperimentData


class OfficialAgentStackRunnerTest(unittest.TestCase):
    def test_candidate_oof_exact_coverage_and_fold_exclusion(self):
        ...
        data = ExperimentData(
            ...
            base, targets, folds, None,
        )
        bundle = AgentStackData(data, features, {"records": 10}, {"candidates": 30})
        calls = []
        ...
        def fake(spec, train_features, train_base, train_targets, predict_features, predict_base, *, device):
            ...
            return AgentStackFitResult(np.asarray(predict_base, dtype=np.float64), {"coefficient_sha256": str(len(calls))})

        with patch("mal2026.iterative_official_agent_stack_runner.fit_predict_agent_stack", side_effect=fake):
