...
from mal2026.iterative_official_balanced_boundary_models import BalancedBoundaryResult, candidate_specs
from mal2026.iterative_official_balanced_boundary_runner import (
    OfficialBalancedBoundaryData,
    _candidate_oof,
    load_official_balanced_boundary_data,
    run_outer_fold,
)
from mal2026.iterative_tail_runner import ExperimentData


def bundle_fixture() -> OfficialBalancedBoundaryData:
...
class OfficialBalancedBoundaryRunnerTest(unittest.TestCase):
    def test_score_free_canonical_hash_map_and_agent_source_split(self):
        ...
        with (
            patch("mal2026.iterative_official_balanced_boundary_runner.load_protocol", return_value=protocol),
            patch("mal2026.iterative_official_balanced_boundary_runner.validate_bound_inputs"),
            patch("mal2026.iterative_official_balanced_boundary_runner.load_experiment_data", return_value=experiment),
            patch("mal2026.iterative_official_balanced_boundary_runner.load_writing_rows", return_value=writings) as score_free,
            patch("mal2026.iterative_official_balanced_boundary_runner.load_dual_candidates", side_effect=fake_load),
            patch("mal2026.iterative_official_balanced_boundary_runner.build_dual_agent_features", side_effect=fake_features),
        ):
            result = load_official_balanced_boundary_data()
    ...
    def test_candidate_oof_exact_coverage_and_fold_exclusion(self):
        ...
        def fake(spec, train_features, train_base, train_targets, predict_features, predict_base, *, device):
            ...
            return BalancedBoundaryResult(np.asarray(predict_base, dtype=np.float64), {"fresh_initialization": True})

        with patch("mal2026.iterative_official_balanced_boundary_runner.fit_predict_balanced_boundary", side_effect=fake):
    ...
    def test_outer_shard_completes_all_three_before_baseline_fallback(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            with (
                patch("mal2026.iterative_official_balanced_boundary_runner.PUBLIC_ROOT", Path(temp) / "public"),
                patch("mal2026.iterative_official_balanced_boundary_runner.RESTRICTED_ROOT", Path(temp) / "restricted"),
                patch("mal2026.iterative_official_balanced_boundary_runner.load_protocol", return_value=SimpleNamespace(raw={})),
                patch("mal2026.iterative_official_balanced_boundary_runner.validate_bound_inputs"),
                patch("mal2026.iterative_official_balanced_boundary_runner.load_official_balanced_boundary_data", return_value=bundle),
                patch("mal2026.iterative_official_balanced_boundary_runner._candidate_oof", side_effect=fake_oof),
                patch("mal2026.iterative_official_balanced_boundary_runner.select_candidate", side_effect=fake_select),
                patch("mal2026.iterative_official_balanced_boundary_runner.compute_iterative_tail_metrics", return_value={"macro": {"rmse": .5}}),
            ):
    ...
    def test_outer_shard_refits_only_frozen_selected_candidate(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            with (
                ...
                patch("mal2026.iterative_official_balanced_boundary_runner._candidate_oof", side_effect=fake_oof),
                patch("mal2026.iterative_official_balanced_boundary_runner.select_candidate", side_effect=fake_select),
                patch("mal2026.iterative_official_balanced_boundary_runner._outer_refit", return_value=(np.full((2, 3), 3.25), {"fresh_initialization": True})) as refit,
                patch("mal2026.iterative_official_balanced_boundary_runner.compute_iterative_tail_metrics", return_value={"macro": {"rmse": .5}}),
            ):
                result = run_outer_fold(0, device="cuda:0")
