...
from mal2026.iterative_official_selective_flip_models import candidate_specs
from mal2026.iterative_official_selective_flip_protocol import CONFIG_PATH
from mal2026.iterative_official_selective_flip_selection import final_gate, score5_macro_recall, select_candidate
...
class OfficialSelectiveFlipSelectionTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_incomplete_inventory_fails_closed_to_baseline(self):
    ...
    def test_score5_recall_remains_descriptive(self):
    ...
    def test_final_gate_requires_strict_negative_candidate_minus_baseline_upper(self):
        candidate = metrics(.55, .68, .90, .85, .66, .62)
        passed = final_gate(self.config, self.baseline, candidate, {"candidate_minus_baseline_ci": {"upper": -.001}})
