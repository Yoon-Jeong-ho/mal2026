"""Sealed 5x4 nested runner for the V9 official Terra selective-flip heads."""
...
from .iterative_official_selective_flip_models import (
    SelectiveFlipSpec,
    candidate_specs,
    fit_predict_selective_flip,
)
from .iterative_official_selective_flip_protocol import RUN_ID, load_protocol, validate_bound_inputs
from .iterative_official_selective_flip_selection import final_gate, fold_direction_diagnostics, select_candidate
...
PUBLIC_ROOT = Path("outputs/iterative-official-selective-flip-v9") / RUN_ID
RESTRICTED_ROOT = Path("data/processed/restricted/iterative_official_selective_flip_v9") / RUN_ID
SEED = 2026080209


class OfficialSelectiveFlipRunError(RuntimeError):
    """Raised when V9 nesting, coverage, selection, or privacy differs."""


@dataclass(frozen=True)
class OfficialSelectiveFlipData:
...
def load_official_selective_flip_data() -> OfficialSelectiveFlipData:
    ...
    if audit["records"] != 2000 or provenance["candidates"] != 6000 or provenance["human_or_reference_score_read_or_prompted"] is not False:
        raise OfficialSelectiveFlipRunError("official feature provenance differs")
    return OfficialSelectiveFlipData(experiment, features, audit, provenance)


def _candidate_oof(
    bundle: OfficialSelectiveFlipData,
    ...
    *,
    ...
    for inner_validation in universe:
        ...
        if outer_fold in data.folds[train] or outer_fold in data.folds[predict] or inner_validation in data.folds[train]:
            raise OfficialSelectiveFlipRunError("V9 inner/outer fold sealing differs")
    ...
    if (
        ...
        raise OfficialSelectiveFlipRunError("V9 candidate OOF coverage differs")
...
def run_outer_fold(outer_fold: int, *, device: str = "cuda:0") -> Mapping[str, Any]:
    if outer_fold not in range(5):
        raise OfficialSelectiveFlipRunError("outer fold must be 0..4")
    protocol = load_protocol()
    validate_bound_inputs(protocol)
    bundle = load_official_selective_flip_data()
    ...
    for position, spec in enumerate(specs, start=1):
        ...
        _write_json(progress_path, {
            "schema_version": "mal2026-iterative-official-selective-flip-progress-v9",
    ...
    result = {
        "schema_version": "mal2026-iterative-official-selective-flip-outer-v9",
        "status": "completed",
        "outer_fold": outer_fold,
...
def _read_outer_predictions(data: ExperimentData) -> tuple[np.ndarray, np.ndarray, list[Mapping[str, Any]]]:
    ...
    for outer_fold in range(5):
        ...
        if not result_path.is_file() or not prediction_path.is_file():
            raise OfficialSelectiveFlipRunError("V9 outer artifact missing")
        ...
        if result.get("status") != "completed" or result.get("candidate_count") != 3 or result.get("restricted_prediction_sha256") != _sha256(prediction_path):
            raise OfficialSelectiveFlipRunError("V9 outer artifact binding differs")
        count = 0
        with prediction_path.open(encoding="utf-8") as stream:
            for line in stream:
                ...
                if source_id not in index or source_id in seen or row.get("outer_fold") != outer_fold:
                    raise OfficialSelectiveFlipRunError("V9 outer row identity differs")
                position = index[source_id]
                if int(data.folds[position]) != outer_fold:
                    raise OfficialSelectiveFlipRunError("V9 outer row fold differs")
                ...
                if base_vector.shape != (3,) or selected_vector.shape != (3,) or not np.isfinite(base_vector).all() or not np.isfinite(selected_vector).all():
                    raise OfficialSelectiveFlipRunError("V9 outer prediction vector differs")
                baseline[position], selected[position] = base_vector, selected_vector
                seen.add(source_id); count += 1
        if count != 400:
            raise OfficialSelectiveFlipRunError("V9 outer row count differs")
        audits.append({
            "outer_fold": outer_fold,
    ...
    if len(seen) != 2000 or not np.isfinite(baseline).all() or not np.isfinite(selected).all():
        raise OfficialSelectiveFlipRunError("V9 concatenated outer coverage differs")
...
def aggregate_outer_results() -> Mapping[str, Any]:
    ...
    if not np.allclose(baseline, data.base.astype(np.float64), rtol=0.0, atol=1e-7):
        raise OfficialSelectiveFlipRunError("V9 nested baseline differs from exact R0")
    ...
    _write_jsonl(final_path, [
        {
            ...
            "role": "nested_selected_official_selective_flip_stack" if final_pass else "exact_r0_oof_baseline_fallback",
        }
        for row, source_id in enumerate(data.source_ids)
    ])
    payload = {
        "schema_version": "mal2026-iterative-official-selective-flip-aggregate-v9",
        "status": "completed",
        "record_count": 2000,
        "outer_fold_count": 5,
        ...
        "final_selection": "nested-selected-official-selective-flip-stack" if final_pass else "exact-r0-oof-baseline-fallback",
        ...
        "feature_audit": bundle.feature_audit,
        "official_candidate_provenance": bundle.provenance,
        "adaptive_after_v8_observed": True,
        "v9_full_oof_prestudy_before_preregistration": False,
    ...
    completion = {
        "schema_version": "mal2026-iterative-official-selective-flip-completion-v9",
...
def gpu0_smoke(*, device: str = "cuda:0") -> Mapping[str, Any]:
    ...
    for spec in candidate_specs():
        ...
        if result.predictions.shape != (32, 3) or not np.isfinite(result.predictions).all():
            raise OfficialSelectiveFlipRunError("V9 smoke prediction differs")
        ...
        if len(residual_hash) != 64 or not head_hashes or any(len(str(value)) != 64 for value in head_hashes):
            raise OfficialSelectiveFlipRunError("V9 smoke coefficient hash differs")
        records.append({"variant_id": spec.variant_id, "audit": result.audit})
    payload = {
        "schema_version": "mal2026-iterative-official-selective-flip-smoke-v9",
...
__all__ = [
    "PUBLIC_ROOT", "RESTRICTED_ROOT", "SEED", "OfficialSelectiveFlipData", "OfficialSelectiveFlipRunError",
    "aggregate_outer_results", "gpu0_smoke", "load_official_selective_flip_data", "run_outer_fold",
]
