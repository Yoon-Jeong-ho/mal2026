"""Sealed 5x4 nested runner for the V11 official Terra/Luna agent heads."""
...
from .iterative_official_dual_agent_models import FEATURE_DIM, build_dual_agent_features
from .iterative_official_balanced_boundary_models import (
    BalancedBoundarySpec,
    candidate_specs,
    fit_predict_balanced_boundary,
)
from .iterative_official_balanced_boundary_protocol import RUN_ID, load_protocol, validate_bound_inputs
from .iterative_official_balanced_boundary_selection import final_gate, fold_direction_diagnostics, select_candidate
...
PUBLIC_ROOT = Path("outputs/iterative-official-balanced-boundary-v11") / RUN_ID
RESTRICTED_ROOT = Path("data/processed/restricted/iterative_official_balanced_boundary_v11") / RUN_ID
SEED = 2026080211


class OfficialBalancedBoundaryRunError(RuntimeError):
    """Raised when V11 nesting, coverage, selection, or privacy differs."""


@dataclass(frozen=True)
class OfficialBalancedBoundaryData:
...
def load_official_balanced_boundary_data() -> OfficialBalancedBoundaryData:
    ...
    if len(essay_sha256_by_source) != 2000 or set(essay_sha256_by_source) != set(experiment.source_ids):
        raise OfficialBalancedBoundaryRunError("canonical score-free essay SHA population differs")
    ...
    features, audit = build_dual_agent_features(
        experiment.base, experiment.source_ids, terra, luna,
    )
    if (
        audit.get("records") != 2000 or audit.get("dimensions") != FEATURE_DIM
        or audit.get("human_or_reference_score_read_or_prompted") is not False
        ...
        raise OfficialBalancedBoundaryRunError("official balanced-boundary feature provenance differs")
    return OfficialBalancedBoundaryData(experiment, features, audit, provenance)


def _candidate_oof(
    bundle: OfficialBalancedBoundaryData,
    ...
    *,
    ...
    for inner_validation in universe:
        ...
        if outer_fold in data.folds[train] or outer_fold in data.folds[predict] or inner_validation in data.folds[train]:
            raise OfficialBalancedBoundaryRunError("V11 inner/outer fold sealing differs")
        result = fit_predict_balanced_boundary(
    ...
    if (
        ...
        raise OfficialBalancedBoundaryRunError("V11 candidate OOF coverage differs")
...
def run_outer_fold(outer_fold: int, *, device: str = "cuda:0") -> Mapping[str, Any]:
    if outer_fold not in range(5):
        raise OfficialBalancedBoundaryRunError("outer fold must be 0..4")
    protocol = load_protocol()
    validate_bound_inputs(protocol)
    bundle = load_official_balanced_boundary_data()
    ...
    for position, spec in enumerate(specs, start=1):
        ...
        _write_json(progress_path, {
            "schema_version": "mal2026-iterative-official-balanced-boundary-progress-v11",
    ...
    result = {
        "schema_version": "mal2026-iterative-official-balanced-boundary-outer-v11",
        "status": "completed",
        "outer_fold": outer_fold,
...
def _read_outer_predictions(data: ExperimentData) -> tuple[np.ndarray, np.ndarray, list[Mapping[str, Any]]]:
    ...
    for outer_fold in range(5):
        ...
        if not result_path.is_file() or not prediction_path.is_file():
            raise OfficialBalancedBoundaryRunError("V11 outer artifact missing")
        result = json.loads(result_path.read_text(encoding="utf-8"))
        if (
            result.get("schema_version") != "mal2026-iterative-official-balanced-boundary-outer-v11"
            ...
            raise OfficialBalancedBoundaryRunError("V11 outer artifact binding differs")
        count = 0
        with prediction_path.open(encoding="utf-8") as stream:
            for line in stream:
                ...
                if source_id not in index or source_id in seen or row.get("outer_fold") != outer_fold:
                    raise OfficialBalancedBoundaryRunError("V11 outer row identity differs")
                position = index[source_id]
                if int(data.folds[position]) != outer_fold:
                    raise OfficialBalancedBoundaryRunError("V11 outer row fold differs")
                ...
                if base_vector.shape != (3,) or selected_vector.shape != (3,) or not np.isfinite(base_vector).all() or not np.isfinite(selected_vector).all():
                    raise OfficialBalancedBoundaryRunError("V11 outer prediction vector differs")
                baseline[position], selected[position] = base_vector, selected_vector
                seen.add(source_id); count += 1
        if count != 400:
            raise OfficialBalancedBoundaryRunError("V11 outer row count differs")
        audits.append({
            "outer_fold": outer_fold,
    ...
    if len(seen) != 2000 or not np.isfinite(baseline).all() or not np.isfinite(selected).all():
        raise OfficialBalancedBoundaryRunError("V11 concatenated outer coverage differs")
...
def aggregate_outer_results() -> Mapping[str, Any]:
    ...
    if not np.allclose(baseline, data.base.astype(np.float64), rtol=0.0, atol=1e-7):
        raise OfficialBalancedBoundaryRunError("V11 nested baseline differs from exact R0")
    ...
    bootstrap = {
        "schema_version": "mal2026-iterative-official-balanced-boundary-bootstrap-v11",
        **bootstrap,
    }
    ...
    _write_jsonl(final_path, [
        {
            ...
            "role": "nested_selected_balanced_boundary_stack" if final_pass else "exact_r0_oof_baseline_fallback",
        }
    ...
    payload = {
        ...
        "final_selection": "nested-selected-balanced-boundary-stack" if final_pass else "exact-r0-oof-baseline-fallback",
        ...
        "feature_audit": bundle.feature_audit,
        "official_candidate_provenance": bundle.provenance,
        "adaptive_after_v10_observed": True,
        "v11_full_oof_prestudy_before_preregistration": False,
...
def gpu0_smoke(*, device: str = "cuda:0") -> Mapping[str, Any]:
    ...
    for spec in candidate_specs():
        ...
        if result.predictions.shape != (32, 3) or not np.isfinite(result.predictions).all():
            raise OfficialBalancedBoundaryRunError("V11 smoke prediction differs")
        ...
        if (len(residual_hash) != 64 or (spec.head_kind != "identity" and not head_hashes)
            ...
            raise OfficialBalancedBoundaryRunError("V11 smoke coefficient hash differs")
        records.append({"variant_id": spec.variant_id, "audit": result.audit})
    payload = {
        "schema_version": "mal2026-iterative-official-balanced-boundary-smoke-v11",
...
__all__ = [
    "PUBLIC_ROOT", "RESTRICTED_ROOT", "SEED", "OfficialBalancedBoundaryData", "OfficialBalancedBoundaryRunError",
    "aggregate_outer_results", "gpu0_smoke", "load_official_balanced_boundary_data", "run_outer_fold",
]
